...
# 定义评测和可视化函数
def evaluate_img_cls(image_info, modal_list=['bbox', 'contour']):
...
def evaluate_grading(image_info):
    true_list, pred_list = [], []
    for img, info in image_info.items():
        try:
            true_list.append(get_grade_label(info['g_true']))
            pred_list.append(get_grade_label(info['g_pred']))
        except:
            print(f'Exception at {img}:')
            raise
    print('HCC grading results:')
    print(metrics.classification_report(true_list, pred_list, digits=4, zero_division=0))



def main(img_dir, q_file, a_file, vis_dir, patch_flag):
    qjsons, ajsons = [], []
    ...
    # 评测分级性能
    evaluate_grading(image_info)

def get_args():
    parser = argparse.ArgumentParser(description='evaluate HCC grading',
    ...
    parser.add_argument('--patch', action='store_true', help='only eval grading.')
    return parser.parse_args()

if __name__ == '__main__':
    args = get_args()
    main(args.img_dir, args.q_file, args.a_file, args.vis_dir, args.patch)