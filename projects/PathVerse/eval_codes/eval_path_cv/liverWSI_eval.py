...
# 定义评测和可视化函数
def evaluate_img_cls(image_info, category, modal_list=['bbox', 'contour']): #modal_list=['patch', 'bbox', 'contour']
...
def visualize_bbox_contour(img_dir_format, vis_dir_format, category, image_info):
    img_dir = img_dir_format.format(c=category)
    vis_dir = vis_dir_format.format(c=category)
...
def main(img_dir_format, q_file_format, a_file_format, vis_dir_format):
    for category in ['edge01']:#, 'edge02', 'edge04', 'edge08'
        #print(f'-------------------{category}-------------------')
        qjsons, ajsons = [], []
        with open(q_file_format.format(c=category), 'r') as file:
            for line in file: qjsons.append(json.loads(line))
        with open(a_file_format.format(c=category), 'r') as file:
        ...
        # 可视化bbox和contour
        if vis_dir_format:
            visualize_bbox_contour(img_dir_format, vis_dir_format, category, image_info)
...
if __name__ == '__main__':
    args = get_args()
    main(args.img_dir_format, args.q_file_format, args.a_file_format, args.vis_dir_format)