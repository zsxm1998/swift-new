...
if [ "$#" -lt 1 ]; then
  echo "Usage: $0 <CKPT_DIR> [-ng] [--think] [--func] [--loc] [-tp <int>] [--batch-size <int>]"
  exit 1
fi
...
TASK_NAME="13_PathMMU_test_tiny_choice"
RES_FILE="$LOG_DIR/$TASK_NAME.log"
QUESTION_FILE="datasets_public/PathMMU/test_tiny.json"
...
# Perform evaluation
echo -e '—————————————————————————————————— PathMMU Test Tiny Performance ——————————————————————————————————' > "$RES_FILE"
python projects/PathVerse/eval_codes/nips_eval/pathmmu_choice_eval.py \
  --result_file "$ANSWER_FILE" \
  --gt_file "$QUESTION_FILE" >> "$RES_FILE"
